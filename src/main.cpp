...
void SPIFFS_INIT(void)
{
   ...
   if (SPIFFS.exists(SPIFFS_MQTT_ID_FILE))  // fist time with new config
   {
      loadConfiguration(myConfig); // load fallback config here and overwite by legacy config
      String myName = readSpiffs(SPIFFS_MQTT_ID_FILE);

      if (myName != "")
      {
         myConfig.name = myName.c_str();
      }
   ...
   {
      loadConfiguration(myConfig); // load normal config
   }
   ...
   if (sensorCalib != "")
   {
      ...
      if (splitSensorDataString(sensorCalib, &offset, &factor))
      {
         ...
         myConfig.calibF = factor;
         myConfig.calibO = offset;
...
void SPIFFS_MAIN(void)
{
   ...
   {
      myConfig.name = myMqttHelper.getName().c_str();
